...
int main() {
	sf::RenderWindow window(sf::VideoMode(640, 480), "Aula 4 - Construindo animações"); // criando uma janela através da class Window

	sf::Event event;

	sf::Texture textura; //criando uma textura
	textura.loadFromFile("./resources/mega.png"); //atribuindo uma imagem a textura
	...
	while (window.isOpen()) { // enquanto a janela estiver aberta - o loop de eventos
		...
		if (clock.getElapsedTime().asSeconds() > 0.2f) { // a cada quantidade de clock, 0.2f define também a velocidade em que a imagem será atualizada
			...
			if (rect.left == 255) { // se o lado esquerdo do rect estiver na seguinte posição, 255
